...
from models import User, Group, Role, UserGroup
from auth.utils import get_current_active_user
from dao import GroupManager, UserGroupManager, UserManager, RoleManager
...
async def join_group(
	...
	usergroup = UserGroup(user_id=user.id, group_id=group_id)
	await UserGroupManager.create(session, usergroup)
	await session.commit()
...
async def leave_group(
	...
	await UserGroupManager.delete_by(session, user_id=user.id, group_id=group_id)
	await session.commit()
